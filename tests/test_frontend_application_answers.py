...
def test_bulk_cover_letter_button_includes_partial_failures() -> None:
    source = FRONTEND_SOURCE.read_text()

    assert "function autoprepCoverLetterIsRegeneratable(job)" in source
    assert '["ready", "failed", "interrupted"].includes(job.cover_letter_status)' in source
...
def test_application_answer_regeneration_supports_per_answer_tweak_drafts() -> None:
    ...
    assert '<link rel="stylesheet" href="/assets/app.css?v=vanilla-20260915-27" />' in index
    assert '<script type="module" src="/assets/app.js?v=vanilla-20260915-27"></script>' in index
