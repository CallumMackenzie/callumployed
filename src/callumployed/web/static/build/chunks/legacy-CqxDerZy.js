...
`,`<br>`)}</div></details>`:``}
    <div class="prepped-document-grid">
      ${Sa(t,`resume`,`Resume`)}
      ${Sa(t,`cover-letter`,`Cover letter`)}
    </div>
    ...
    <p class="prepped-safety-note">Autoprep prepares files only. It never submits an application.</p>`}async function xa(e,t){let n=`${e.role_id}:${t}`,r=_a(e,t);if(!(Gt.get(n)===r||qt.has(n))){Q(n),qt.add(n),Kt.delete(n),ba();try{let i=await Dn(`/api/autoprep/roles/${encodeURIComponent(e.role_id)}/documents/${t}.pdf?v=${encodeURIComponent(e.updated_at||``)}`),a=N.find(t=>Number(t.role_id)===Number(e.role_id));if(!L.has(n)||!a||_a(a,t)!==r){URL.revokeObjectURL(i),L.delete(n);return}R.set(n,i),Gt.set(n,r)}catch(e){Kt.set(n,e instanceof Error?e.message:`PDF preview unavailable`)}finally{qt.delete(n),Number(P)===Number(e.role_id)&&ba()}}}function Sa(e,t,n){let r=t===`cover-letter`?`cover_letter`:`resume`,i=e[`${r}_status`],a=e[`${r}_artifact_path`],o=a?.split(`/`).pop()??`Not available`,s=e[`${r}_error`],c=e[`${r}_instruction`]||``,l=`${e.role_id}:${t}`,u=Wt.get(l)??c,d=t===`cover-letter`?`Optional comments for the next version`:`Comments for the next version`,ee=t===`cover-letter`?`Optionally describe specific, truthful changes...`:`Describe specific, truthful changes...`,te=[`failed`,`interrupted`].includes(i),f=e.worker_state!==`idle`||[`queued`,`generating`,`generating_tweaks`,`regenerating`].includes(i),ne=!f&&(i===`ready`||te)&&(te||t===`cover-letter`||String(u).trim()),p=L.has(l),m=`/api/autoprep/roles/${encodeURIComponent(e.role_id)}/documents/${t}.pdf?v=${encodeURIComponent(e.updated_at||``)}`,h=R.get(l),re=Kt.get(l),ie=qt.has(l),ae=a?`<a class="prep-cover-pdf-link" data-autoprep-view="${t}" href="${H(m)}" target="_blank" rel="noreferrer" aria-label="View ${H(n.toLowerCase())} PDF in browser">View PDF</a>`:``;return`
    <section class="prepped-document${p?` has-open-preview`:``} status-${H(i)}">
      <div class="prepped-document-heading"><h4>${H(n)}</h4><span>${H(ga(i))}</span></div>
