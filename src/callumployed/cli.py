...
from callumployed.config import BrowserSettings, LlmSettings
...
def set_config_command(
    ...
    """Set a web setting; profile changes refresh prepared cover letters after 30 seconds."""
    try:
        with db.connect() as connection:
    ...
    if key in APPLICANT_KEYS:
        typer.echo(
            "Prepared cover letters will refresh after 30 quiet seconds; "
            "applications are never submitted or marked Applied."
        )
...
def _configured_scan_options() -> tuple[BrowserProfileManager, LlmSettings]:
    connection = db.connect()
    try:
        return (
            configured_browser_profile_manager(connection),
            configured_llm_settings(connection),
        )
    finally:
        connection.close()
...
def scan_url_command(
    ...
    browser_profile_manager, llm_settings = _configured_scan_options()
    try:
        result = asyncio.run(
            run_scan_url(
                url,
...
def rescan_role_command(
    ...
    browser_profile_manager, _llm_settings = _configured_scan_options()
    try:
        result = asyncio.run(
            run_rescan_role(
                role_id,
