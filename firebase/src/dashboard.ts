  ...
  return page("Callumployed metrics", `
    ...
    <main class="dashboard">
      ...
      <section class="grid">
        ...
        <article class="panel wide"><p class="eyebrow">Companies</p><h2>Performance</h2><div class="table-wrap"><table><thead><tr><th data-company-sort-header="company_name" aria-sort="none"><button type="button" class="sort-button" data-company-sort="company_name">Company <span class="sort-indicator" aria-hidden="true"></span></button></th><th data-company-sort-header="scans" aria-sort="descending"><button type="button" class="sort-button" data-company-sort="scans">Scans <span class="sort-indicator" aria-hidden="true">↓</span></button></th><th data-company-sort-header="success_rate" aria-sort="none"><button type="button" class="sort-button" data-company-sort="success_rate">Success <span class="sort-indicator" aria-hidden="true"></span></button></th><th data-company-sort-header="average_duration_ms" aria-sort="none"><button type="button" class="sort-button" data-company-sort="average_duration_ms">Avg duration <span class="sort-indicator" aria-hidden="true"></span></button></th><th data-company-sort-header="candidates_scanned" aria-sort="none"><button type="button" class="sort-button" data-company-sort="candidates_scanned">Candidates <span class="sort-indicator" aria-hidden="true"></span></button></th><th data-company-sort-header="roles_discovered" aria-sort="none"><button type="button" class="sort-button" data-company-sort="roles_discovered">Discovered <span class="sort-indicator" aria-hidden="true"></span></button></th><th data-company-sort-header="roles_saved" aria-sort="none"><button type="button" class="sort-button" data-company-sort="roles_saved">Saved <span class="sort-indicator" aria-hidden="true"></span></button></th><th data-company-sort-header="failed_role_visits" aria-sort="none"><button type="button" class="sort-button" data-company-sort="failed_role_visits">Failed visits <span class="sort-indicator" aria-hidden="true"></span></button></th></tr></thead><tbody id="companies"></tbody></table></div></article>
    ...
    <script>
      ...
      const compareCompanyRows = ${compareCompanyRows.toString()};
      const companySort = {key:'scans', direction:'desc'};
      let companyRows = [];
      ...
      function render(data) {
        ...
        const cards = [
          ...
          ['Agent-assisted scans', number(s.agent_assisted_scans), percent(s.total_scans ? s.agent_assisted_scans / s.total_scans : 0)],
        ];
        ...
        companyRows = data.companies; renderTimeline(data.timeseries); renderFunnel(s); renderBars('#failures', data.failures, 'No failed scans'); renderBars('#versions', data.versions, 'No version data'); renderCompanies();
...
export function compareCompanyRows(
  ...
  if (key === "company_name") {
    comparison = String(left[key] ?? "").localeCompare(String(right[key] ?? ""), undefined, {
      sensitivity: "base",
      numeric: true,
    });
  } else {
  ...
  if (comparison === 0 && key !== "company_name") {
    comparison = String(left.company_name ?? "").localeCompare(
      String(right.company_name ?? ""), undefined, {sensitivity: "base", numeric: true},
    );
  }
